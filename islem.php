...
if (isset($_POST['yemkayitekle'])) {
	$yemkayit_adi = $_POST['yemkayit_adi'];
	$yemkayit_miktari = $_POST['yemkayit_miktari'];
	...
	foreach ($yemkayit_adi as $key => $yemkayitAdi) {
		...
		if ($ekle && $guncelle) {
			...
			header("Location:production/yem-kayit.php?durum=false");
		}
	}
}
...
if (isset($_POST['yemkayitguncelle'])) {
	...
	$yemkayitguncelle = $db->prepare("UPDATE yem_kayit SET
...
#ırk ayarı - ırk ekle
#
if (isset($_POST['irkeklesigir'])) {

	$irkekle = $db->prepare("INSERT INTO irk SET
			irk_adi=:adi,
			irk_ozellikleri=:aciklama");
	$ekle = $irkekle->execute(array(
		'adi' => $_POST['irk_adi'],
		'aciklama' => $_POST['irk_ozellikleri'],
	));
	if ($ekle) {
		header("Location:production/irklar.php?durum_sigir=true");
	} else {
		header("Location:production/irklar.php?durum_sigir=false");
	}
}

#
#ırk ayarı - ırk güncelle
#
if (isset($_POST['irkguncellesigir'])) {
...
#ırk ayarı - ırk ekle
#
if (isset($_POST['irkeklekoyun'])) {

	$irkekle = $db->prepare("INSERT INTO irk_koyun SET
			irk_adi=:adi,
			irk_ozellikleri=:aciklama");
	$ekle = $irkekle->execute(array(
		'adi' => $_POST['irk_adi'],
	...
	if ($ekle) {
		header("Location:production/irklar.php?durum_koyun=true");
	} else {
		header("Location:production/irklar.php?durum_koyun=false");
	}
}
...
#ırk ayarı - ırk güncelle
#
if (isset($_POST['irkguncellekoyun'])) {
